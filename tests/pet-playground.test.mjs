...
function fixture() {
  let clock = 0, id = 0, blocked = false;
  ...
  const engine = context.api.createPlayground({ root, cat: { offsetWidth: 96 },
    ...
    state: name => states.push(name), busy: () => blocked || doc.hidden });
  ...
  return { engine, states, frames, timers, doc, win, api: context.api, frame, finish, next,
...
test('scrolling down and up causes immediate opposite trots and schedules exploration', () => {
  ...
  assert.ok([...f.timers.values()].some(t => t.delay === 250));
  f.next(); f.finish();
